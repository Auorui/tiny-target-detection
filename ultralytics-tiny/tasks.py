...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            args = [ch[f]]
        elif m in (Concat, FFMConcat):   # MODIFIED: merged Concat and FFMConcat
            ...
            c2 = ch[f[-1]]
        elif m in (FCM, HFP):             # MODIFIED: merged FCM/HFP duplicate logic
            c1 = c2 = ch[f]               # both input and output use same channel count
            args = [c1, *args[0:]]
        elif m is ESSamp:                 # MODIFIED: add ESSamp module support
            c1, c2 = args[0], args[1]
        elif m is SDP:                    # MODIFIED: add SDP module support
            c1 = c2 = ch[-1]              # in/out channels from previous layer
            args = [c1, *args[0:]]
        elif m in (PKIStage, PKIStem):    # MODIFIED: add PKIStage/PKIStem module support
            c1, c2 = ch[f], args[0]       # c1 from input layer, c2 from args[0]
            args = [c1, c2, *args[1:]]
        elif m is AVG:                    # MODIFIED: add AVG module support
            if not args:                  # set default pooling size if args is empty
                args = [2]
            c2 = ch[f]
        elif m in (SCAM, MKP, LAM):       # MODIFIED: add support for SCAM/MKP/LAM modules
            c2 = ch[f]                    # input channels = output channels for these modules
            args = [c2]
        elif m is CSFM:                   # MODIFIED: add CSFM module support
            c_shallow, c_mid, c_deep = [ch[x] for x in f]
            c2 = c_mid    # Must be written. output channels = mid-level channels
            args = [c_shallow, c_mid, c_deep]
        elif m is ASFF3:                 # MODIFIED: add ASFF3 module support
            level = args[0]
        ...
        elif m is involution:            # MODIFIED: add involution module support
            # involution: [channels, kernel_size, stride]
            c1 = c2 = ch[f]              # in/out channels from input (channel-preserving)
            args = [c2, *args[1:]]       # rebuild args: [channels, kernel_size, stride]
