...
from ultralytics.nn.extra_modules.norm_block import to_2tuple
...
class DilatedReparamBlock(nn.Module):
    ...
    def merge_dilated_branches(self):
        if hasattr(self, 'origin_bn'):
            origin_k, origin_b = self.fuse_bn(self.lk_origin, self.origin_bn)
            for k, r in zip(self.kernel_sizes, self.dilates):
                conv = self.__getattr__('dil_conv_k{}_{}'.format(k, r))
                bn = self.__getattr__('dil_bn_k{}_{}'.format(k, r))
                branch_k, branch_b = self.fuse_bn(conv, bn)
...
class DepthBottleneckUniv2(nn.Module):
    def __init__(self,
        ...
        if use_depthwise:
            ...
            self.act = nn.SiLU()
            self.one_conv = Conv(mid_channel, mid_channel2, k=1)

            self.conv3 = UniRepLKNetBlock(mid_channel2, kernel_size=kersize)
            self.act1 = nn.SiLU()
            self.one_conv2 = Conv(mid_channel2, out_channels, k=1)
        else:
